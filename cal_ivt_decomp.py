...
def match_daily_climatology(data, clim):
    ...
    This works for climatology files with either:
    - 365 daily values
    - 366 daily values

    It also avoids xarray coordinate conflicts by using numpy indices.
    """
    ...
    # Case 1: climatology has 366 days
    if nclim == 366:
    ...
    # Case 2: climatology has 365 days
    # Remove Feb 29 from leap years and shift days after Feb 28
...
for year in years:
    ...
    # Make sure q, u, and v use the same time axis
    ...
    # Thermodynamic/moisture contribution: q' * V_clim
    ...
    ds_out["IVT_dyn"].attrs["long_name"] = (
        "Dynamic contribution to IVT anomaly: climatological moisture times anomalous wind"
    )
    ds_out["IVT_thermo"].attrs["long_name"] = (
        "Thermodynamic contribution to IVT anomaly: anomalous moisture times climatological wind"
    )
    ds_out["IVT_nonlinear"].attrs["long_name"] = (
        "Nonlinear contribution to IVT anomaly: anomalous moisture times anomalous wind"
    )
    ...
    ds_out.attrs["description"] = (
        ...
        "IVT_dyn, IVT_thermo, and IVT_nonlinear are magnitudes of vector contributions. "
        "They should not be summed directly."
    )
